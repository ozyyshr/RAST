...
class DExpertsLlama:
    def __init__(
        ...
        self.expert1 = AutoModelForCausalLM.from_pretrained(
            expert_model1, **model_kwargs
        )
        self.antiexpert1 = AutoModelForCausalLM.from_pretrained(
            antiexpert_model1, **model_kwargs
        )
        ...
        print(f"decoding with alpha: {self.alpha}", flush=True)
    ...
    def forward(
        ...
        if expert1_inputs is not None and antiexpert1_inputs is not None:
            expert1_outputs = self.expert1(**expert1_inputs, return_dict=return_dict)
            antiexpert1_outputs = self.antiexpert1(**antiexpert1_inputs, return_dict=return_dict)

            return base_outputs, expert_outputs, antiexpert_outputs, expert1_outputs, antiexpert1_outputs
        else:
    ...
    def generate(
        ...
        **kwargs
        ...
        expert1_kwargs = kwargs.copy()
        antiexpert1_kwargs = kwargs.copy()
        ...
        if self.use_chat_format_for_expert:
            ...
            expert_input_ids1 = expert_input_ids.clone()
        ...
        antiexpert_input_ids1 = input_ids.clone()
        ...
        for step in range(max_new_tokens):
            ...
            expert1_inputs = self.expert1.prepare_inputs_for_generation(expert_input_ids1, **expert1_kwargs)
            antiexpert1_inputs = self.antiexpert1.prepare_inputs_for_generation(antiexpert_input_ids1, **antiexpert1_kwargs)

            # DExperts
            if expert1_inputs is not None and antiexpert1_inputs is not None:
                base_outputs, expert_outputs, antiexpert_outputs, expert1_outputs, antiexpert1_outputs = self.forward(
                    base_inputs, expert_inputs, antiexpert_inputs,
                    expert1_inputs=expert1_inputs,
                    antiexpert1_inputs=antiexpert1_inputs,
                    return_dict=True
                )
            else:
            ...
            if expert1_outputs is not None and antiexpert1_outputs is not None:
                expert1_next_token_logits = expert1_outputs.logits[..., -1, :]
                antiexpert1_next_token_logits = antiexpert1_outputs.logits[..., -1, :]

            assert expert_next_token_logits.shape == antiexpert_next_token_logits.shape

            # print('base_next_token_logits', base_next_token_logits.shape, flush=True) # torch.Size([bz, 152064])
            # print('expert_next_token_logits', expert_next_token_logits.shape, flush=True) # torch.Size([bz, 151936])
            # print('antiexpert_next_token_logits', antiexpert_next_token_logits.shape, flush=True) # torch.Size([bz, 151936])

            # truncate base_next_token_logits to the size of the expert model
            larger_shape = max(base_next_token_logits.shape[-1], expert_next_token_logits.shape[-1])
            if base_next_token_logits.shape[-1] != larger_shape:
                flag = True
                ...
                # TODO: errorneous padding
                expert1_next_token_logits = torch.cat([expert1_next_token_logits, torch.zeros(expert1_next_token_logits.shape[0], 128).to(expert1_next_token_logits.device)], dim=1)
                antiexpert1_next_token_logits = torch.cat([antiexpert1_next_token_logits, torch.zeros(antiexpert1_next_token_logits.shape[0], 128).to(antiexpert1_next_token_logits.device)], dim=1)
            assert base_next_token_logits.shape == expert_next_token_logits.shape

            # sometimes our experts have extra (irrelevant) tokens at the end of the normal vocabulary
            expert_next_token_logits = expert_next_token_logits[:, :base_next_token_logits.shape[-1]]

            print(base_next_token_logits)
            print(expert_next_token_logits)
            print(antiexpert_next_token_logits)
            print(expert1_next_token_logits)
            print(antiexpert1_next_token_logits)
            ...
            next_token_logits = (
                ...
                (1 - self.alpha) * (expert1_next_token_logits - antiexpert1_next_token_logits)
            )
            ...
            if do_sample:
                ...
                # pad to original vocab size for Qwen
                if not flag:
                    probs = torch.cat([probs, torch.zeros(probs.shape[0], 128).to(probs.device)], dim=1)
